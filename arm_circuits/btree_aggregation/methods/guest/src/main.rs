...
///  ///  The circuit of a single step in a binary tree aggregation.
...
fn main() {
    ...
    let (hh_in, dd_in): (Digest, Digest) = env::read();
        ...
        && dd_in != Digest::ZERO
    {
        ...
        env::verify(
            ag_program_key,
            &serde::to_vec(&(ag_program_key, hh_in, dd_in)).unwrap(),
        )
        .unwrap();
    }

    // Hash-chain the verified output and program.
    let h_out = arm::hash::commit_step_output_with_sha(&[h_in, hh_in], &step_output);
    let d_out = arm::hash::commit_step_program_with_sha(&[d_in, dd_in], &step_program);
