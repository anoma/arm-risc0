...
fn parse_delta_point(
    ...
    // We must check the curve equation manually: y^2 = x^3 + 7 (mod p)
...
pub fn verify_delta_proof(tx: &Transaction) -> Result<(), ArmError> {
    ...
    // Solana's secp256k1_recover expects raw recid (0 or 1).
    let recid = signature_bytes[64]
        .checked_sub(27)
        .ok_or(ArmError::InvalidDeltaProof)?;
...
mod tests {
    ...
    use crate::delta_types::DeltaProof;
    use crate::Digest;
